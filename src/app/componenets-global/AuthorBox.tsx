...
import { getDefaultThemeColor } from './defaults'
...
export default function AuthorBox({ authorName }: AuthorBoxProps) {
  ...
  return (
    <div className={`border border-gray-200 rounded-lg p-6 mt-8 bg-gradient-to-br from-${themeColor}-50 to-white shadow-md mx-4`}>
      <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6">
        ...
        <div className="flex-grow text-center sm:text-left">
          ...
          <div className="mt-4 flex justify-center sm:justify-start space-x-4">
            <a href={author.twitter} target="_blank" rel="noopener noreferrer" className={`text-${themeColor}-500 hover:text-${themeColor}-600 transition-colors`}>
              <Twitter size={20} />
            </a>
            <a href={author.linkedin} target="_blank" rel="noopener noreferrer" className={`text-${themeColor}-500 hover:text-${themeColor}-600 transition-colors`}>
              <Linkedin size={20} />
            </a>
          </div>
        </div>
