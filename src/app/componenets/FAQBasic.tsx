...
export default function FAQBasic({ title, items, style = "default" }: FAQProps) {
  ...
  return (
    <section
      id="faq-section"
      className={`w-full mx-auto [&:not(#breadcrumb+&)]:py-12 [#breadcrumb+&]:pt-0 [#breadcrumb+&]:pb-12 px-4 ${style === "default" ? "max-w-4xl" : ""}`}
    >
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaMarkup) }} />
      <div className="text-center mb-8">
        <h2 className={`text-3xl font-bold font-serif text-primary ${style === "style2" ? "text-left" : ""}`}>
          {title}
        </h2>
          ...
          <div className="space-y-4">
            {items.map((item, index) => (
              <div key={index} className="border-b border-primary/20 pb-4">
                ...
                >
                  <h3 className="text-lg font-medium font-serif text-primary">{item.question}</h3>
                  <Plus
                ...
                >
                  <p className="text-foreground/80 italic">{item.answer}</p>
                </div>
              </div>
            ))}
          </div>
