                  ...
                  <ul className="space-y-2">
                    {column.map((item) => (
                      <li key={item}>
                        <Link 
                          href={
                            urlPrefix && urlPrefix.includes("http")
                              ? `${urlPrefix}${urlPrefix.endsWith("/") ? "" : "/"}${item.toLowerCase().replace(/\s+/g, "-")}`
                              : `/${urlPrefix ? `${urlPrefix}/` : ""}${item.toLowerCase().replace(/\s+/g, "-")}`
                          }
                          className="text-primary hover:text-primary/80 transition-colors duration-200 text-sm font-serif"
                          {...(urlPrefix && urlPrefix.includes("http") ? { target: "_blank", rel: "noopener" } : {})}
                        >
                          {item}
                        </Link>
                      </li>
